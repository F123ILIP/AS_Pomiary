...
class MyDatabaseHelper_Brzuch extends SQLiteOpenHelper {
    private Context context;
    private static final String DATABASE_NAME = "Pomiary_Brzuch.db";
    private static final int DATABASE_VERSION = 2;
    private static final String TABLE_NAME = "moje_pomiary_Brzuch";
    private static final String COLUMN_ID = "_id_Brzuch";
    private static final String COLUMN_VALUE = "value_Brzuch";
    private static final String COLUMN_DATE = "date_Brzuch";

    MyDatabaseHelper_Brzuch(@Nullable Context context) {
    ...
    void updateData(String row_id, String value, String date) {
        ...
        try {
            int result = db.update(TABLE_NAME, cv, "_id_Brzuch=?", new String[]{row_id});
    ...
    void deleteOneRow(String row_id) {
        SQLiteDatabase db = this.getWritableDatabase();
        try {
            int result = db.delete(TABLE_NAME, "_id_Brzuch=?", new String[]{row_id});
            // Zwracaj wynik lub obsłuż ewentualny błąd
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
